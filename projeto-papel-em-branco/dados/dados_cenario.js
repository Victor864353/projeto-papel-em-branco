...
function sortearIndividual(categoria) {
    const lista = dados_cenario[categoria]; // So vai alterar o "dados_personagem" quando for reutilizar
    if (lista) {
...
function sortearTudo() {
    for (const categoria in dados_cenario) { // So vai alterar o "dados_personagem" quando for reutilizar
